# main_enhanced.py - Phase 6 (Enhanced Accuracy & Signal Quality)
import os
import asyncio
import aiohttp
import time
...
SIGNAL_CONF_THRESHOLD = float(os.getenv("SIGNAL_CONF_THRESHOLD", 75.0))  # Increased to 75%
...
CANDLE_URL = "https://api.binance.com/api/v3/klines?symbol={symbol}&interval=30m&limit=100"  # Increased limit
...
def detect_patterns(candles) -> Dict[str, bool]:
    ...
    patterns['hammer'] = (lower_wick > 2 * body_size) and (upper_wick < body_size)
        ...
        try:
            ...
            if bids and asks:
                ...
                if total_ask_volume > 0:
                    out["buy_pressure"] = total_bid_volume / total_ask_volume
                else:
                    out["buy_pressure"] = 0
        except Exception as e:
...
def enhanced_plot_chart(times, candles, symbol, market_data):
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8), dpi=100, height_ratios=[3, 1])
...
def analyze_market_structure(candles, current_price: float) -> dict:
    ...
    # Trend analysis using higher highs/lower lows
    recent_highs = []
    recent_lows = []
    ...
    return {
        ...
        "strength": len(recent_highs) + len(recent_lows)  # More levels = stronger structure
    }
